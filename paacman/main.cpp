		...
		while (!game_over)
		{
			...
			for (lives; lives > 0;)
			{
				...
				while (stop == false)
				{
					...
					Player_G(maze, direction, tmp_direction, G, eat, P_B_R, stop, exit);
					...
					if (difficulty == 1)
					{
						...
						if (score > 30)
						{
							Reset(maze, grid_B, 1, 252, P_B_R, G);
							Random_path(maze, stop, grid_P, P_B_R, 1, 82, G, eat);
						}

						if (score > 100)
						{
							Reset(maze, grid_R, 2, 251, P_B_R, G);
							Random_path(maze, stop, grid_P, P_B_R, 2, 66, G, eat);
						}
					}
					...
					else
					{
						...
						if (rand_P.rand > 0)
						{
							...
							if (eat == false)
								Find_path(maze, stop, grid_P, 80, P_B_R, 0, G, eat);
							else
								Random_path(maze, stop, grid_P, P_B_R, 0, 80, G, eat);
						...
						if (score > 30)
						{
							...
							if (rand_B.rand > 0)
							{
								...
								if (eat == false)
									Find_path(maze, stop, grid_B, 82, P_B_R, 1, G, eat);
								else
									Random_path(maze, stop, grid_P, P_B_R, 1, 82, G, eat);
						...
						if (score > 100)
						{
							...
							if (rand_R.rand > 0)
							{
								...
								if (eat == false)
									Find_path(maze, stop, grid_R, 66, P_B_R, 2, G, eat);
								else
									Random_path(maze, stop, grid_P, P_B_R, 2, 66, G, eat);
				...
				if (lives == 0)
				{
					...
					if (size == 0)
					{
						...
						cin.getline(buf, 10);
						strcpy_s(players[size].name, buf);
					...
					else
					{
						...
						for (int i = 0, j = 0; j < size; i++)
						{
							if (pos == i)
							{
								...
								strcpy_s(tmp[j].name, buf);
								tmp[j].best_score = highscore;

								strcpy_s(tmp[j + 1].name, players[i].name);
							...
							if (pos == -1 && j == size - 1)
							{
								...
								strcpy_s(tmp[size - 1].name, buf);
								tmp[size - 1].best_score = highscore;

								break;
							}

							strcpy_s(tmp[j].name, players[i].name);
						...
						for (int i = 0; i < size; i++)
						{
							...
							strcpy_s(players[i].name, tmp[i].name);
						}
					}
